...
//
Logger.prototype.log = function (level, msg, showLevel) {
  var self = this,
      callback,
      meta;
  ...
  if (this.stripColors) {
	...
	if (showLevel === undefined || showLevel === null) {showLevel = true;}
  for (var i = 0, l = this._names.length; i < l; i++) {
    ...
    if ((transport.level && self.levels[transport.level] <= self.levels[level]) || (!transport.level && self.levels[self.level] <= self.levels[level])) {
        transport.log(showLevel ? level : '', msg, meta, function (err) {
